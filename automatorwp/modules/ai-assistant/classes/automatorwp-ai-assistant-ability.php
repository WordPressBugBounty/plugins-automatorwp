 ...
 * AI Ability
 *
 * @package     AutomatorWP\Classes\AI_Ability
...
class AutomatorWP_AI_Assistant_Ability {
    ...
    public function get_response_prefix( $success = true ) {
        ...
        $prefixes = array(
            __( 'Success!', 'automatorwp' ),
            __( 'Done!', 'automatorwp' ),
            __( 'Ready!', 'automatorwp' ),
            __( 'Complete!', 'automatorwp' ),
            __( 'Completed!', 'automatorwp' ),
            __( 'Finish!', 'automatorwp' ),
            __( 'Finished!', 'automatorwp' ),
            __( 'It\'s done!', 'automatorwp' ),
            __( 'I\'ve finished.', 'automatorwp' ),
        );

        if( ! $success ) {
            $prefixes = array(
                __( 'Oops!', 'automatorwp' ),
                __( 'Failed!', 'automatorwp' ),
                __( 'Couldn\'t complete.', 'automatorwp' ),
                __( 'Something went wrong.', 'automatorwp' ),
            );
        }
    ...
    public function get_user( $search = '' ) {
        ...
        if( count( $user_ids ) === 0 ) {
            return $this->response_error( sprintf(
                __( 'I couldn\'t find the user "%s".', 'automatorwp' )
                . ' ' . __( 'Try providing a different name, email or another identifying detail.', 'automatorwp' ),
                $search
            ) );
            ...
            return $this->response_error( sprintf(
                __( 'I found several users matching with "%s".', 'automatorwp' )
                . ' ' . __( 'Try providing a different name, email or another identifying detail.', 'automatorwp' ),
                $search
            ) );
        }
    ...
    public function get_users( $search = '' ) {
        ...
        if( count( $user_ids ) === 0 ) {
            return $this->response_error( sprintf(
                __( 'I couldn\'t find any user by "%s".', 'automatorwp' )
                . ' ' . __( 'Try providing a different name, email or another identifying detail.', 'automatorwp' ),
                $search
            ) );
        }
    ...
    public function get_post( $search = '', $args = array() ) {
        ...
        if( count( $post_ids ) === 0 ) {
            return $this->response_error( sprintf(
                ...
                . ' ' . __( 'Try providing a different title, slug or another identifying detail.', 'automatorwp' ),
    ...
    public function get_posts( $search = '', $args = array() ) {
        ...
        if( count( $post_ids ) === 0 ) {
            return $this->response_error( sprintf(
                // translators: %1$s: Post type plural %2$s: Search term
                __( 'I couldn\'t find any %s "%s".', 'automatorwp' )
                . ' ' . __( 'Try providing a different title, slug or another identifying detail.', 'automatorwp' ),
                $post_type->labels->name,
                $search
            ) );
