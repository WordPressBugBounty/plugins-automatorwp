 ...
 * Ajax Functions
 *
 * @package     AutomatorWP\AI_Assistant\Ajax_Functions
 * @since       1.0.0
 * @version     1.0.0
 */
 ...
 * @since   1.0.0
 */
function automatorwp_ai_assistant_ajax_process_prompt() {
    ...
    if( empty( $prompt ) )
        wp_send_json_error( __( 'Please, type something.', 'automatorwp' ) );

    // Check if supports AI
    $supports_ai = function_exists('wp_supports_ai') && (bool) call_user_func('wp_supports_ai');

    if( ! $supports_ai )
        wp_send_json_error( __( 'This assistant requires an AI connector in order to work, please configure at least one.', 'automatorwp' ) );

    // Check model
    $model = isset( $_POST['model'] ) ? sanitize_text_field( $_POST['model'] ) : '';

    if( ! empty( $model ) ) {
        $models = automatorwp_ai_assistant_get_models_ids();
    ...
    try {
        ...
        wp_send_json_error( __( 'Could not process your prompt.', 'automatorwp' ) );
    }
    ...
    foreach( $history as $prev ) {
        ...
        try {
            ...
            // wp_send_json_error( __( 'Could not process history messages.', 'automatorwp' ) );
        }
    }

    $history_messages[] = $user_message;

    $abilities = automatorwp_ai_assistant_get_abilities();

    $prompt = wp_ai_client_prompt()
        ->using_system_instruction( automatorwp_ai_assistant_get_system_instructions() )
    ...
    if ( ! $prompt->is_supported_for_text_generation() )
        wp_send_json_error( sprintf( __( 'No models found that support %s for this prompt.', 'automatorwp' ), __( 'text generation', 'automatorwp' ) ) );
     ...
     * @return integer
     */
    $max_calls = apply_filters( 'automatorwp_ai_assistant_max_ability_calls', 5 );
    $response = '';
    ...
    while ( $call < $max_calls ) {
        ...
        try {
            ...
            // Do not report this error
            // wp_send_json_error( array( 'message' => __( 'Could not process AI result.', 'automatorwp' ) ) );
        }
        ...
        if ( $resolver->has_ability_calls( $message ) ) {
            ...
            $function_responses = automatorwp_ai_assistant_get_all_function_responses( $response_message );
...
add_action( 'wp_ajax_automatorwp_ai_assistant_process_prompt', 'automatorwp_ai_assistant_ajax_process_prompt' );