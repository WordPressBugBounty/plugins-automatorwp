...
$.ajax({url:ajaxurl,method:'POST',data:{action:'automatorwp_delete_automation_item',nonce:automatorwp_admin.nonce,id:id,item_type:item_type},success:function(response){if(response.success){if(item_type==='trigger'||item_type==='action'){$('.automatorwp-automation-tag-selector optgroup[data-id="'+id+'"]').remove()}}else{}},error:function(response){}})});$('body').on('click','.automatorwp-automation-item-action-close',function(e){var item=$(this).closest('.automatorwp-automation-item');var items_list=$(this).closest('.automatorwp-automation-items');var item_type=item.hasClass('automatorwp-trigger')?'trigger':'action';var id=item.find('input.id').val();item.slideUp('fast',function(){item.remove()})});if($('body.edit-anonymous-automation').length){if($('#automatorwp_triggers .automatorwp-automation-item').length===0){$('<div class="automatorwp-automation-item automatorwp-trigger">'+$('#automatorwp_triggers').find('.automatorwp-add-item-form').html()+'</div>').appendTo('.automatorwp-triggers')}
...
if((item_type==='trigger'||item_type==='action')&&response.data.tags_html.length){$(response.data.tags_html).appendTo('.automatorwp-automation-tag-selector')}
...
var first_change=row.hasClass('is-option-change');var value=$(this).val();if(value==='custom'||(Array.isArray(value)&&value.includes('custom'))){if(first_change){custom_input_row.show()}else{custom_input_row.slideDown('fast')}}else{if(first_change){custom_input_row.hide()}else{custom_input_row.slideUp('fast')}}
...
$this.addClass('disabled');$this.closest('.automatorwp-dialog-buttons').prepend('<span class="spinner is-active" style="float: none;"></span>');var type='user';var selected=$('.automatorwp-automation-type-dialog .automatorwp-automation-type-selected');if(selected.length){type=selected.data('type')}
...
$this.prop('disabled',!0);$this.find('.automatorwp-cancel-automation-run-cancel-label').hide();$this.find('.automatorwp-cancel-automation-run-cancelling-label').show();automatorwp_cancel_automation_run=!0})})(jQuery);function automatorwp_add_new_item_from_select(select){var $=$||jQuery;if(select.prop('disabled')){return}
var item=select.closest('.automatorwp-automation-item');var items_list=item.closest('.automatorwp-automation-items');var automation_id=$('input#object_id').val();var type=select.val();var item_type=item.hasClass('automatorwp-trigger')?'trigger':'action';if(!type.length){return}
...
select.prop('disabled',!0);items_list.siblings('.automatorwp-spinner').show();$.ajax({url:ajaxurl,method:'POST',data:{action:'automatorwp_add_automation_item',nonce:automatorwp_admin.nonce,automation_id:automation_id,item_type:item_type,type:type,position:position},success:function(response){items_list.siblings('.automatorwp-spinner').hide();item.remove();if(response.success){var new_item=$(response.data.edit_html);new_item.appendTo(items_list);if(item_type==='trigger'&&$('#sequential').prop('checked')){automatorwp_update_items_position(items_list);items_list.find('.automatorwp-automation-item:not(.automatorwp-automation-item-filter) .automatorwp-automation-item-position').show()}
if((item_type==='trigger'||item_type==='action')&&response.data.tags_html.length){$(response.data.tags_html).appendTo('.automatorwp-automation-tag-selector')}
automatorwp_initialize_form_fields(new_item)}else{}},error:function(response){}})}
function automatorwp_update_items_position(list){var $=$||jQuery;var items=list.find('.automatorwp-automation-item');var items_order={};var current_position=0;items.each(function(index,value){$(this).find('input.position').val(index);if(!$(this).hasClass('automatorwp-automation-item-filter')){$(this).find('.automatorwp-automation-item-position').html((current_position+1)+'.');current_position++}
items_order[$(this).find('input.id').val()]=index});if(Object.entries(items_order).length){var item_type=list.hasClass('automatorwp-triggers')?'trigger':'action';$.ajax({url:ajaxurl,method:'POST',data:{action:'automatorwp_update_automation_items_order',nonce:automatorwp_admin.nonce,items_order:items_order,item_type:item_type,},success:function(response){if(response.success){}else{}},error:function(response){}})}
automatorwp_update_tags_selector_order()}
function automatorwp_update_tags_selector_order(){var $=$||jQuery;var items=$('.automatorwp-automation-item');var items_order={};var triggers_sep=!1;var actions_sep=!1;var tags_index=0;var prefix='t:';items.each(function(index,value){if($(this).hasClass('automatorwp-trigger')){prefix='t:'}else{prefix='a:'}
if(!triggers_sep&&$(this).hasClass('automatorwp-trigger')){triggers_sep=!0;items_order[tags_index]='triggers_sep';tags_index++}
if(!actions_sep&&$(this).hasClass('automatorwp-action')){actions_sep=!0;items_order[tags_index]='actions_sep';tags_index++}
items_order[tags_index]=prefix+$(this).find('input.id').val();tags_index++});if(Object.entries(items_order).length){$.each(items_order,function(index,id){$('.automatorwp-automation-tag-selector').each(function(){if(id==='triggers_sep'||id==='actions_sep'){$(this).find('option[value="'+id+'"]').appendTo(this)}else{$(this).find('optgroup[data-id="'+id+'"]').appendTo(this)}})})}}
...
if($(this).attr('type')==='checkbox'||$(this).attr('type')==='radio'){if($(this).closest('.cmb2-checkbox-list').length){value=[];$(this).closest('.cmb2-checkbox-list').find('input:checked').each(function(){value.push($(this).val())})}else if(!$(this).prop('checked')){return}}else if($(this).closest('.cmb-row.cmb-type-text-datetime-timestamp').length){value={'date':$(this).closest('.cmb-row.cmb-type-text-datetime-timestamp').find('input.cmb2-datepicker').val(),'time':$(this).closest('.cmb-row.cmb-type-text-datetime-timestamp').find('input.cmb2-timepicker').val(),}}else if($(this).hasClass('wp-editor-area')){option=$(this).attr('name');if(window.tinyMCE.editors.length&&window.tinyMCE.editors[$(this).attr('id')]){window.tinyMCE.editors[$(this).attr('id')].save();value=$(this).val()}}else if($(this).hasClass('cmb2-upload-file')||$(this).hasClass('cmb2-upload-file-id')){value=$(this).closest('.cmb-td').find('.cmb2-upload-file').val()}
