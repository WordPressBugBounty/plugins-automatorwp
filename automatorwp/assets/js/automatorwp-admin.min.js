...
$.ajax({url:ajaxurl,method:'POST',data:{action:'automatorwp_delete_automation_item',nonce:automatorwp_admin.nonce,id:id,item_type:item_type},success:function(response){if(response.success){if(item_type==='trigger'||item_type==='action'){$('.automatorwp-automation-tag-selector optgroup[data-id="'+id+'"]').remove()}}else{}},error:function(response){}})});if($('body.edit-anonymous-automation').length){if($('#automatorwp_triggers .automatorwp-automation-item').length===0){$('<div class="automatorwp-automation-item automatorwp-trigger">'+$('#automatorwp_triggers').find('.automatorwp-add-item-form').html()+'</div>').appendTo('.automatorwp-triggers')}
