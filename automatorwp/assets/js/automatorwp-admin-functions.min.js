...
var formatted_results=[];var results=(response.data.results!==undefined?response.data.results:response.data);results.forEach(function(item){formatted_results.push(jQuery.extend({id:item.id,text:item.text,},item))});return{results:formatted_results,pagination:{more:(response.data.more_results!==undefined?response.data.more_results:!1)}}}
function automatorwp_matches_by_word(text,term){text=text.toUpperCase();var words=term.toUpperCase().split(" ");var found=!0;for(var i=0;i<words.length;i++)
found=(found&&text.indexOf(words[i])>=0);return found}
function automatorwp_select2_words_matcher(params,data){if(params.term===undefined||params.term.trim()==='')return data;if(automatorwp_matches_by_word(data.text,params.term))return data;return null}
function automatorwp_select2_optgroup_matcher(params,data){data.parentText=data.parentText||'';if(params.term===undefined||params.term.trim()==='')return data;if(data.children&&data.children.length>0){var match=jQuery.extend(!0,{},data);for(var c=data.children.length-1;c>=0;c--){var child=data.children[c];child.parentText+=data.parentText+" "+data.text;var matches=automatorwp_select2_optgroup_matcher(params,child);if(matches==null){match.children.splice(c,1)}}
if(match.children.length>0){return match}
return automatorwp_select2_optgroup_matcher(params,match)}
if(automatorwp_matches_by_word((data.parentText+' '+data.text),params.term))return data;return null}
...
function automatorwp_is_valid_url(url){var result=url.match(/(http(s)?:\/\/.)?(www\.)?[-a-zA-Z0-9@:%._\+~#=]{2,256}\.[a-z]{2,10}\b([-a-zA-Z0-9@:%_\+.~#?&//=]*)/g);return(result!==null)}